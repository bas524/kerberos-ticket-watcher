...
Principal::Principal(CCache& cCache) : _context(cCache.context()) {
  ...
  if (retval != 0) {
    throw KRB5EXCEPTION(retval, "Cant' get principal from ccache");
  }
}
...
Principal Principal::buildTargetPrincipal() const {
  ...
  if (retval != 0) {
    throw KRB5EXCEPTION(retval, "Can't build target principal");
  }
...
QString Principal::name() const {
  ...
  if (retval != 0) {
    throw KRB5EXCEPTION(retval, "Error while unparsing principal name");
  }
